...
def main():
    ...
    k_vertical = np.array([[-1, 0, 1]]).T.dot(np.array([[1, 1, 1]]))
    k_horizontal = np.rot90(k_vertical)
    Ix = my_convolve(gray, k_horizontal)
    Iy = my_convolve(gray, k_vertical)
    plt.imshow(Ix, cmap='gray')
    plt.imshow(Iy, cmap='gray')
    plt.imsave('/home/haochen/Projects/ImageProcessing100Wen/Question_11_20/my_output/16_prewitt_ix.png', Ix, cmap='gray')
    plt.imsave('/home/haochen/Projects/ImageProcessing100Wen/Question_11_20/my_output/16_prewitt_iy.png', Iy, cmap='gray')
    plt.show()
