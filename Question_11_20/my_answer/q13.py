...
def my_convolve(img: np.ndarray, kernel: np.ndarray) -> np.ndarray:
    ...
    for y in range(H):
        for x in range(W):
            for c in range(C):
                out[pad + y, pad + x, c] = ((tmp[y: y + k, x: x + k, c]).max() - (tmp[y: y + k, x: x + k, c]).min())
...
def main():
    ...
    kernel = np.eye(3) / 3  # motion filter
    out = my_convolve(img, kernel)
    plt.imshow(out)
    plt.imsave('/home/haochen/Projects/ImageProcessing100Wen/Question_11_20/my_output/13.png', out)
    plt.show()
